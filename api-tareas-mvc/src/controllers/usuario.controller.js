...
/**
 * POST /api/usuarios - Crear usuario desde panel de administración
 */
const crearDesdeAdmin = async (req, res) => {
  try {
    ...
    if (!['admin', 'usuario', 'visualizador'].includes(rol)) {
      return res.status(400).json({
        success: false,
    ...
    const usuario = await db.Usuario.create({ nombre, email, password, rol, activo });
...
const buscar = async (req, res) => {
  try {
    const { nombre, email, rol, activo, tagIds } = req.query;
    const ids = parseIds(tagIds);
