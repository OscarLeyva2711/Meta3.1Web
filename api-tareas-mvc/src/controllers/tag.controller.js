...
const buscar = async (req, res) => {
  try {
    const { nombre, usuarioIds } = req.query;
    const ids = parseIds(usuarioIds);

    if (!nombre && !ids.length) {
      return res.status(400).json({
        success: false,
        message: 'Proporciona un parámetro de búsqueda: nombre o usuarioIds'
      });
    }

    if (ids.length && req.user?.rol !== 'admin') {
      return res.status(403).json({
        success: false,
        message: 'Sólo un administrador puede buscar etiquetas por usuarios'
      });
    }
